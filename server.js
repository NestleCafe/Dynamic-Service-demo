...
var server = http.createServer(function(request, response) {
  ...
  if (path === "/sign_in" && method === "POST") {
    ...
    let sessionId;
    try {
        //一个不用正则但是麻烦的方法寻找user_id
      sessionId = cookie
        .split(";") //此时为cookie为 ['logined=1','user_id','2']
        .filter(s => s.indexOf("session_id=") >= 0)[0]  // 取出user_id 部分的字符串（此时为=开头）,此时为['user_id','2'] 
        .split("=")[1] //同理以=号split ，取出2
    } catch (error) {}
